...
/**
 * Neutral zone responsible of individual alignment.
 */
class NeutralZone(
    ...
    override fun getNextMovement(): Euclidean2DPosition {
        ...
        return if (positions.contains(RelativeLateralPosition.LEFT) &&
            !positions.contains(RelativeLateralPosition.RIGHT)
        ) {
            movement + movementProvider.toLeft()
        } else if (!positions.contains(RelativeLateralPosition.LEFT) &&
            positions.contains(RelativeLateralPosition.RIGHT)
        ) {
