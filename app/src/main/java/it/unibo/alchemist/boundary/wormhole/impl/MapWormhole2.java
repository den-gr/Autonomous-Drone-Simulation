 ...
 *
 */
public final class MapWormhole2 extends WormholeSwing<GeoPosition> {
...
//    @SuppressFBWarnings(value = "EI_EXPOSE_REP2", justification = "This is intentional")
    public MapWormhole2(final Environment<?, GeoPosition> environment, final Component comp, final IMapViewPosition m) {
        super(environment, comp);
        mapModel = m;
        mapModel.moveCenter(36.0, 36.0);
    ...
    public void center() {
        ...
        final PointAdapter<GeoPosition> center = Double.isNaN(off[0]) || Double.isNaN(off[1]) || size[0] <= 0 || size[1] <= 0
                ? from(30, 30)
                : from((off[0] + size[0] / 2), (off[1] + size[1] / 2));
        setEnvPosition(center.toPosition(super.getEnvironment()));
    }
}